...
- [x] 配置拆分：`messages.yml`、`settings/config.yml`、`levels/*.yml`、`resources.yml`、`transitions.yml`、`rooms.yml`、`loot.yml`、`worldgen.yml`。
...
- [x] Loot Source MVP：原版 `CHEST` / `BARREL` 容器打开时可按配置注入 Loot Table，并用 TileState PDC 标记 one-time 生成状态。
