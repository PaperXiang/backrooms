...
- [x] `/br base force-claim <id> <uuid> [name]` 与 `/br base unclaim <id>` 已接入管理员验证流程，可无在线玩家测试 Base claim 持久化与清理。
- [x] Transition MVP：region 触发、右键方块触发预留、Level/world 目标、冷却、传送后免触发保护。
- [x] `/br transitions`、`/br transition info <id>`、`/br transition trigger <id> [player]`、`/br transition guide <id>`、`/br transition test <id> <world> <x> <y> <z>`。
- [x] Room 占位生成 MVP：`room` / `corridor` 模板、材质 palette、单次方块上限、默认只替换空气。
- [x] `/br rooms`、`/br room info <id>`、`/br room generate <id> [level]`、`/br room generate-at <id> <level> <x> <y> <z>`。
...
- [x] `/br resource harvest <id> [break|right-click]` 已接入，可在无在线玩家时触发配置坐标的 Resource 方块、真实掉落、cooldown 和 remove-block 路径。
...
- [x] `/br verify craftengine` 已扩展 Faithful 静态行为检查，可汇总 `faithful_*` 方块数量、state allocation、非遮挡装饰配置、灯具 luminance 和 crate storage 配置。
...
`/br verify craftengine` 会检查 CraftEngine `backrooms` 配置里的 item/block 定义、BackroomsCore item 是否都有 CE 镜像、`/ce menu` 分类引用、server-side l10n、client-side lang、预制模型引用、Faithful 方块 state/非遮挡装饰/灯具 luminance/crate storage 静态配置，以及旧 `faithfulbackrooms:` 命名空间残留。当前测试服该命令已全 PASS。
...
/br resource harvest level0_loose_carpet
/br resource info level1_scrap_ore
/br resource sample level1_scrap_ore 10
/br resource harvest level1_scrap_ore break
```
...
`/br resource harvest <id> [break|right-click]` 会修改测试服世界：它只作用于配置的固定 `locations`，会按真实方块触发 Resource 产物、启动 cooldown，并执行 `remove-block` / `replacement`。当前测试服已验证 Level 0 右键资源点的掉落与 cooldown，以及 Level 1 break 资源点的掉落与移除方块路径；测试后已恢复 Level 1 占位锚点。
...
无在线玩家时，可用管理员命令验证 claim 文件写入、reload 持久化和清理：

```text
/br base force-claim level1_utility_room_a 00000000-0000-0000-0000-000000000050 RconTest
/br verify bases
/br reload
/br verify bases
/br base unclaim level1_utility_room_a
/br reload
/br verify bases
```

当前测试服已验证：force-claim 后 `runtimeClaims=1, storedClaims=1`，reload 后仍为 1；unclaim 后 reload 回到 `runtimeClaims=0, storedClaims=0`。
...
/br transition test level0_to_level1_stairwell level_0 24 64 -18
/br transition test level1_to_lobby_evacuation level_1 -42 64 -24
```
...
- 无在线玩家时，`/br transition test <id> <world> <x> <y> <z>` 可验证坐标是否命中 transition trigger；当前测试服已验证两个 region 中心点 `matches=true`，外部坐标 `matches=false`。
...
无在线玩家时，可用 RCON 在显式坐标生成测试房间：

```text
/br room generate-at level0_basic_room level_0 96 80 96
/br room generate-at level0_corridor level_0 120 80 96
/br room generate-at level1_utility_room level_1 96 80 96
/br verify rooms
```

当前测试服已验证：Level 0 basic room 改变 364 个方块，Level 0 corridor 改变 341 个方块，Level 1 utility room 改变 532 个方块；随后 `/br verify rooms` 与 `/br verify runtime` 均全 PASS。
